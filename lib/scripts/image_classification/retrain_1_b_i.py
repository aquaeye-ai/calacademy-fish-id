...
#   --output_labels=/home/nightrider/calacademy-fish-id/classifiers/image_classification/models/inception_v3_tfhub_v3/fine_tuned/7_31_2020/1/output_labels.txt \
...
def main(_):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        # store config so we can find the relevant training parameters later
        with open(os.path.join(os.path.dirname(FLAGS.output_graph), 'config.txt'), 'w+') as config_f:
            for flag in vars(FLAGS):
                config_f.write("{}: {}\n".format(flag, getattr(FLAGS, flag)))
